...
async function reclassifyTikTokCreators() {
  ...
  for (const creator of creatorsRes.rows) {
    ...
    const sourceRes = await pool.query(
      ...
      where creator_id = $1 and source_platform = 'tiktok'
    ...
    const hasWebsiteLike = contacts.some((c) =>
      ["website", "youtube", "vimeo", "instagram", "x"].includes(c.contact_type)
    );
...
reclassifyTikTokCreators()
  .then((result) => {
    console.log("TikTok creator reclassification complete:", result);
    process.exit(0);
  })
  .catch((error) => {
    console.error("TikTok creator reclassification failed:", error);
    process.exit(1);
  });
