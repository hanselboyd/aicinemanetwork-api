...
async function findExistingCreator({ username, authorId, normalizedName }) {
  const bySource = await pool.query(
    `
    select c.*
    from creators c
    join creator_sources s on s.creator_id = c.id
    where s.source_platform = 'x'
      and (s.handle = $1 or s.external_id = $2)
    limit 1
    `,
    [username || null, authorId || null]
  );
...
async function saveContacts(creatorId, sourceUrl, contacts) {
  for (const contact of contacts) {
    await pool.query(
      ...
      values ($1, $2, $3, 'x', $4, false, now(), now())
...
async function upsertCreatorFromX(sourceRow, item, classification) {
  const creatorName = item.authorName || item.username || "Unknown Creator";
  ...
  const profileUrl = `https://x.com/${item.username}`;
  ...
  if (!creator) {
    ...
    const inserted = await pool.query(
      ...
      values ($1, $2, 'individual', $3, 'x', $4, $5, $6, now(), false)
      returning *
      `,
  ...
  // Extract contacts from bio and linked URLs
  const bioContacts = extractContacts(item.authorBio || "");
  const linkedContacts = (item.linkedUrls || []).map(url => ({
    type: /youtube|vimeo/i.test(url) ? (url.includes('youtube') ? 'youtube' : 'vimeo') : 'website',
    value: url
  }));

  const allContacts = [
    { type: "x", value: profileUrl },
    ...bioContacts,
    ...linkedContacts
  ];
  ...
  const hasWebsite = contacts.some((c) => ["website", "youtube", "vimeo"].includes(c.type));
